...
int MNIST::read(const std::string& image_file, const std::string& label_file,
                int& n_images, int& width, int& height, int& n_classes,
                std::vector<TrainingData>& training_data)
{
  std::ifstream image_data(image_file.c_str(), std::ios::binary);
  std::ifstream label_data(label_file.c_str(), std::ios::binary);
